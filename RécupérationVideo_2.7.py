...
# /!\ La VRAIE bonne /!\
lower_vert_hsv = np.array([61, 117, 97])
upper_vert_hsv = np.array([117, 255, 188])
...
#lower_joueur = np.array([0, 0, 0])
#upper_joueur = np.array([180, 255, 15])

# Celle pour le joueur mais en rose fluo
lower_joueur = np.array([128, 97, 184])
upper_joueur = np.array([180, 255, 240])
...
def fonction_transfert_degre_angle(x):
    """
        ...
        prendre le premier return pour le premier moteur
        prendre le deuxième return pour le deuxième moteur
    """

    #return(((3*np.pi*x)/(pas_vis*100))*180/np.pi) pour l'ancien montage
    return(5.18*((3*np.pi*x)/(pas_vis*100))*180/np.pi)

def pixel_vers_cm(x):
    """
        float -> float
        crée une bijection entre les pixels de la caméra et la position absolue en cm
    """

    return(a * x + b)

def echelle_pixel_cm(x) :
    """
        float -> float
        renvoie un écart en cm à partir d'un écart en pixels
    """
...
def fonction_transfert_correcteur(x):
    ...
    #if abs(echelle_pixel_cm(x)) > 1.5 :
    return (fonction_transfert_degre_angle(echelle_pixel_cm(x)))
...
def watchMan():
    ...
    if choix_arduino:
        ...
        while not choix_fait:
            ...
            print("6 : La position en FTBF par rapport à la balle")
    ...
    while cap.isOpened():
        ...
        mask_balle = cv.inRange(hsv, lower_actuel, upper_actuel)
        mask_joueur = cv.inRange(hsv, lower_joueur, upper_joueur)

        # On applique une ouverture morphologique avec la matrice définie plus haut pour réduire le bruit de l'image isolé
        opening_balle = cv.morphologyEx(mask_balle, cv.MORPH_OPEN, kernel)
        opening_joueur = cv.morphologyEx(mask_joueur, cv.MORPH_OPEN, kernel)

        # On récupère une liste des contours de l'image (forme fermé)
        contours_b, hierarchy_b = cv.findContours(opening_balle, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE)

        contours_j, hierarchy_j = cv.findContours(opening_joueur, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE)
        if len(contours_b)!=0:
            cnt = contours_b[0]
            ...
            if len(contours_j) !=0:
                #Si on repère du rose, on actualise la position du joueur
                cnt_j = contours_j[0]
                (Cx_j,Cy_j),radius_j = cv.minEnclosingCircle(cnt_j)
                derniere_pos_j_y = Cy_j
                derniere_pos_j_x = Cx_j

            center_j = (int(derniere_pos_j_x), int(derniere_pos_j_y))
            ...
            pos_joueur.append(470 - derniere_pos_j_y)
            ...
            if (time.time() - dernier_mouvement > ecart_mouvements) and choix_arduino:
                ...
                if choix_sortie == 1:
                    ...
                    message = str(fonction_transfert_correcteur(fonction_transfert_sommateur(fit(barre_goal), derniere_pos_j_x)))
                elif choix_sortie == 6 :
                    message = str(fonction_transfert_correcteur(fonction_transfert_sommateur(Cx, derniere_pos_j_x)))
        ...
        mask = cv.resize(mask_balle, size)
        opening = cv.resize(opening_balle, size)
        ...
        """
        cv.imshow('mask', mask_balle)
        cv.imshow('opening', opening_balle)
        cv.imshow('mask_joueur', mask_joueur)
        cv.imshow('opening_joueur', opening_joueur)
        """
...
def exp4():
    ...
    while cap.isOpened():
        ...
        # On convertit l'image au format HSV
        hsv = cv.cvtColor(frame, cv.COLOR_BGR2HSV)
        ...
        message = fonction_transfert_degre_angle(message)
...
def exp5():
    """
        Gère le nécessaire pour l'expérience 5 : contrôle périodique
        du joueur en fonction de la période, de l'amplitude, du cycle
        et de l'offset. L'objectif est de réaliser l'asservissement en
        position.
    """
    ...
    while cap.isOpened():
        ...
        data[3].append( message )
        delta = message - pixel_vers_cm(470-Cy)
        data[4].append(abs(delta))

        message = fonction_transfert_degre_angle(message + delta)
        if t-dernier_instant > ecart_envoi_donnee:
            message = str(message)
            arduino.write(message.encode())
            dernier_instant = t
    ...
    if choix == 1:
        afficher_valeurs_finales(data, 2)
...
def afficher_valeurs_finales(data, source):
    ...
    if source == 0:
        ...
        if choix == 1:
            ...
            plt.plot([barre_goal for i in range(sizey)], [i for i in range(sizey)])
            plt.xlabel('Coordonnée en x (UA)')
            plt.ylabel('Coordonnée en y (UA)')
            plt.show()
        elif choix == 2:
            plt.grid()
        ...
        if choix == 1:
            ...
            plt.plot(data[2], data[0], color = 'red', label ='Réponse')
            plt.plot(data[2], [elem - data[0][0] for elem in data[0]], color = 'orange', label = 'Réponse relative')
            plt.grid()
            plt.legend(loc=1)

            plt.xlabel("Temps en s")
            plt.ylabel("Position du gardien (en cm)")
            plt.show()
        ...
        print("3 - x en fonction du temps")
        print("4 - Position du goal en fonction du temps et consigne en fonction du temps")
        print("5 - Écart à la consigne en fonction du temps")
        print("5 - Exit")
        ...
        if choix == 1:
            ...
            plt.grid()
            plt.plot(data[2], data[4], color = 'blue')
            plt.legend(loc=1)

            plt.xlabel("Temps en s")
            plt.ylabel("Écart à la consigne (en cm), pour une consigne de " + str(max(data[3])))
            plt.show()
...
def etalon():
    """
        None -> float x float
        Met en place l'étalonnage du capteur vidéo entre la valeur en pixel et la position réelle.
        Renvoie les coefficients de la régression linéaire.
    """
    ...
    demarrage = False
    while not demarrage:
        entree = input("Lancement de l'étalonnage ? (y/n)")
        demarrage = (entree == 'y')
    ...
    pos_max = float(input("Quelle est la position max atteinte par le joueur ?"))
    pos_mini = float(input("Quelle est la position mini atteinte par le joueur ?"))

    obs_max, obs_mini = max(data[1]), min(data[1])

    coeff1 = (pos_max - pos_mini) / (obs_max - obs_mini)
    coeff2 = ((pos_max - coeff1 * obs_max) + (pos_min - coeff1 * obs_min)) / 2
...
while going:
    ...
    elif choix_sous_programme == 0:
        print()
        print("Quel étalonnage faire ?")
        print("1 - Auto")
        print("2 - Manuel")
        choix_etalon = int(input("> "))
        if choix_etalon == 1:
            a, b = etalon()
            print('On obtient : a = ' + str(round(a, 4)) + 'et b = ' + str(round(b, 4) + '.')
        elif choix_etalon == 2:
            a = float(input("Valeur de a ?"))
            b = float(input("Valeur de b ?"))
        else:
            print("Problème dans le choix de l'étalonnage... Retour menu.")
            print()
